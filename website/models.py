...
def mail_sending(instance, created, sender, *args, **kwargs):
    if created:
        ...
        message_text = "This is an enquiry mail from website \"flsystems.co.in\" \n" \
                       "Name : {0}\n Subject: {1}\n Email: {2}\n Message: {3}\n, On: {4}".format(
            instance.name, instance.subject, instance.email, instance.message, str(instance.on)
        )
        send_mail(from_email=from_email, recipient_list=['info@flsystems.co.in'], subject='website query from {0}'.format(instance.name),
